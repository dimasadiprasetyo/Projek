...
class DasboardController extends Controller
{
    public function dasboard(){
        ...
        // hari ini
        $total2 = 0;
        $bulan = Trx_header::whereMonth('tgl_trx',date('d'))->with('Pelanggan','Trx_detail','barang')->get();
        foreach ($bulan as $bln) {
        ...
        $bulanpiutang=Trx_header::select(DB::raw("MONTHNAME(tgl_trx)as bulanpiutang"))
                ...
                ->pluck('bulanpiutang');
        // iki pendapatan keseluruhan, masuk e bulanan
        $pendapatan=Trx_header::select(DB::raw("CAST(SUM(total_bayar) as int)as total_bayar"))
                ->GroupBy(DB::raw("Month(tgl_trx)"))
                ->pluck('total_bayar');

        return view('dasboard/index',compact('bulan','Trxheader','total1','total2','total3','piutangdagang','bulanpiutang','pendapatan'));
